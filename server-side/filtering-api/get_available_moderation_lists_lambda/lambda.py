...
from common_src.moderation_lists_util import extract_moderation_lists_from_db, MockDynamoDBTable
...
def local_tasts():
    ...
    assert res['body']['error_msgs'][0] == "<class 'KeyError'>: 'list_id'"

    table = MockDynamoDBTable([{'name': 'X', 'list_id': 'X', 'type': '--', 'description': 'Y'}])
    ...
    table = MockDynamoDBTable([{'name': 'X', 'list_id': 'X', 'type': 'BlackLIst', 'description': 'List1'}])
    event['queryStringParameters']['discourse-provider-id'] = '125'
    res = main_handler(event, None)
    assert res['statusCode'] == 200  # the 125 was not save in the cache because of exception. Now it is saved
    assert "List1" in res['body']

    table = MockDynamoDBTable([{'name': 'X', 'list_id': 'X', 'type': 'BlackLIst', 'description': 'List2'}])
    event['queryStringParameters']['discourse-provider-id'] = '125'
    res = main_handler(event, None)
    assert res['statusCode'] == 200
